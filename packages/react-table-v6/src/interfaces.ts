...
export interface TableProps<D = any, ResolvedData = D>
  extends TextProps,
    ...
    ComponentProps {
  /**
   * 列表要显示的数组或对象，Default: []，内置支持在客户端的排序和分页，也支持server-side data,
   * The data prop should be immutable and only change when you want to update the table
   */
  data: D[];
  /** 若要在data渲染出dom前修改data，就使用函数resolveData，接收data作为唯一参数，返回处理后的数据 */
  ...
  /**
   * 要显示的最少行数，默认值是pageSize，若设置为0，则要注意调整空行样式
   * Default: undefined
  ...
  /** Array of all Available Columns，若属性值也包含columns属性，则会渲染成分组的列 */
...
export interface Column<D = any>
   ...
   * 若传入的是字符串或数组，就会使用默认值，默认值会将传入的数据解析为数组并扁平化，
   * 若数据的key包含点号，则需要提供自定义的accessor函数
   * @example: 'myProperty'
   * @example ["a.b", "c"]
   * @example ["a", "b", "c"]
   * @example {"a": {"b": {"c": $}}}
...
/**
 * column类型可以包含4种常用renderer和一些基本属性
 */
