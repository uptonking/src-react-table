...
class Popover extends Component {
  ...
  getPositioningStyle = (element) => {
    const { anchorEl, anchorReference, marginThreshold, supportsMobile } =
      this.props;
    ...
    warning(
      ...
      [
        ...
        `Some part of it can not be seen on the screen (${(element.height =
          heightThreshold)}px).)`,
  ...
  getAnchorOffset(contentAnchorOffset) {
    const { anchorEl, anchorOrigin, anchorReference, anchorPosition } =
      this.props;
    ...
    return {
      ...
      left:
        anchorRect.left + this.handleGetOffsetLeft(anchorRect, anchorOrigin.x),
    };
  }
  ...
  getTransformOrigin(elemRect, contentAnchorOffset = 0) {
    ...
    return {
      y:
        this.handleGetOffsetTop(elemRect, transformOrigin.y) +
        contentAnchorOffset,
  ...
  render() {
    ...
    const PopoverTransitionProps = isMobile
      ? { direction: 'up' }
      : TransitionProps;
